# envs/stg/stg.tfvars
# 스테이징 환경에 대한 Terraform 변수 값 정의
# 이 파일은 envs/stg/main.tf에서 참조되는 변수들의 실제 값을 제공합니다.
...
# 네트워크 설정
# =============================================================================
# VPC CIDR은 스테이징 환경에 적합한 크기로 설정 (/16 = 65,536개 IP 주소)
...
public_subnets_cidr = [
  ...
  "10.1.1.0/24"  # ap-northeast-2b (256개 IP)
]
...
# 스테이징 환경에 적합한 노드 그룹 설정 (개발보다 더 큰 리소스)
node_groups = {
  # 애플리케이션 워크로드용 노드 그룹
  app = {
    desired_size  = 3           # 스테이징 환경을 위한 더 큰 크기
    min_size      = 2           # 최소 크기 (고가용성 보장)
    max_size      = 6           # 최대 크기 (부하 증가 대비)
    disk_size     = 50          # GB (최소 20GB)
    instance_type = "t3.large"  # 스테이징용 인스턴스 타입 (2 vCPU, 8GB RAM)
  }

  # 모니터링/관측성 도구용 노드 그룹
  monitoring = {
    desired_size  = 2          # 모니터링 도구를 위한 더 많은 노드
    min_size      = 1          # 최소 크기 (고가용성 보장)
    max_size      = 3          # 최대 크기 (리소스 사용량 증가 시 확장)
    disk_size     = 100        # 모니터링 도구를 위한 더 큰 디스크 (메트릭 저장)
    instance_type = "t3.xlarge" # 모니터링용 더 큰 인스턴스 (4 vCPU, 16GB RAM)
  }
}
